...
export function SwapSection() {
    ...
    // HYDRATION FIX: only trust wallet state after mount
    ...
    // Hydration-safe derived wallet state
    const hasAddress = mounted && !!address;
    const isSeiEvm =
        mounted && chainId !== undefined ? chainId === SEI_EVM_CHAIN_ID : true; // treat as OK until mounted
    ...
    const { data: quoteData, isLoading: isQuoteLoading } = useReadContract({
        ...
        args: amountInForQuote !== null ? [amountInForQuote, quotePath] : undefined,
        query: {
    ...
    const frogOutAmount =
        frogOutFormatted && frogOutFormatted.length > 0 ? Number(frogOutFormatted) : 0;

    const toUsdValue =
        frogUsdPrice !== null && frogOutAmount > 0 ? frogOutAmount * frogUsdPrice : null;

    // Disable until mounted so SSR + first client render match (prevents hydration mismatch)
    const swapDisabled =
        !mounted ||
        !hasAddress ||
        wrongNetwork ||
    ...
    async function handleSwap() {
        // keep behavior identical, just respect mounted gate
        if (!mounted) return;
        if (!address) return;
    ...
    // one shared height for both columns
    ...
    return (
        <section className="mx-auto max-w-6xl px-4 pb-14">
            ...
            {/* IMPORTANT: stretch columns to same height on md+ */}
            <div className="mt-4 grid gap-4 md:grid-cols-[2fr_1fr] md:items-stretch">
                ...
                {/* Right: FroggySwap card (fixed height + internal scroll, so it never "hangs" lower than chart) */}
                <div
                    id="swap"
