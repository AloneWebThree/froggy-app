...
export function SwapSection() {
    ...
    const allowedToSymbols: ToSymbol[] = useMemo(() => {
        // Requested: USDY -> FROG and USDY -> SEI
        if (fromSymbol === "USDY") return ["FROG", "SEI"];
        // Existing: SEI -> FROG/USDC/USDY
        return ["FROG", "USDC", "USDY"];
    }, [fromSymbol]);
    ...
    const usdyUsdPrice = useTokenUsdPriceFromRouter({
        ...
        routerAddress: DRAGON_ROUTER_ADDRESS as Address,
        routerAbi: DRAGON_ROUTER_ABI as unknown as Abi,
        tokenDecimals: USDY_DECIMALS, // change if USDY decimals differ
        seiRoute: [WSEI_ADDRESS as Address, ADDR.token as Address, USDY_ADDRESS as Address], // SEI -> FROG -> USDY
    });

    // Amount parsing (SEI-native OR USDY ERC-20)
    ...
    }, [amountInNumber, fromSymbol, seiUsdPrice, usdyUsdPrice]);
    ...
    const v2Path: Address[] = useMemo(() => {
        if (fromSymbol === "SEI") {
            ...
            if (toSymbol === "USDY") return [WSEI_ADDRESS as Address, ADDR.token as Address, USDY_ADDRESS as Address];
            // FROG
            return [WSEI_ADDRESS as Address, ADDR.token as Address];
        }

        // fromSymbol === "USDY"
        if (toSymbol === "FROG") return [USDY_ADDRESS as Address, ADDR.token as Address];
        // USDY -> SEI must end in WSEI for swapExactTokensForSEI
        return [USDY_ADDRESS as Address, ADDR.token as Address, WSEI_ADDRESS as Address];
    }, [fromSymbol, toSymbol]);
    ...
    }, [frogUsdPrice, usdyUsdPrice, seiUsdPrice, quote.outFormatted, tokenOutAmount, toSymbol]);

    // USDY allowance (only relevant when USDY is input)
    ...
    const executeSwap = useCallback(async () => {
        ...
        try {
            ...
            // SEI-native in (existing behavior)
            if (fromSymbol === "SEI") {
            ...
            // USDY in: approve if needed, then swap
            const allowance = typeof usdyAllowance === "bigint" ? usdyAllowance : 0n;
            ...
            // USDY -> SEI (multi-hop USDY -> FROG -> WSEI, router unwraps to SEI)
            await writeContractAsync({
                chainId: SEI_EVM_CHAIN_ID,
                address: DRAGON_ROUTER_ADDRESS as Address,
                abi: DRAGON_ROUTER_ABI as unknown as Abi,
    ...
    const primaryLabel = useMemo(() => {
        ...
        if (fromSymbol === "USDY" && amountInRaw) {
            const allowance = typeof usdyAllowance === "bigint" ? usdyAllowance : 0n;
            if (allowance < amountInRaw) return "Approve & swap";
        }
                            ...
                            <div className="grid gap-2 min-w-0">
                                <div className="flex items-center justify-between text-xs text-brand-subtle">
                                    ...
                                    <span className="font-mono text-brand-text">
                                        {fromSymbol === "SEI" ? "SEI (native)" : "USDY"}
                                    </span>
                                </div>
